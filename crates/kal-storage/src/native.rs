//! Native (desktop / mobile) SQLite-backed implementation of `Database`.
//!
...
//!
//! Compiled for every target except `wasm32`, which uses the IndexedDB-backed
//! in-memory store in `wasm.rs`.
...
/// Enables `?` on rusqlite results inside native methods.
...
// Preserve the reverse conversion used by row mappers.
