...
| D60 | **Live cross-internet sync = iroh-gossip + DHT topic discovery (`distributed-topic-tracker`)**, not file gossip, and not iroh's per-node DNS discovery alone. | iroh-gossip's `subscribe` needs known peer `EndpointId`s, so a plain n0/`PkarrResolver` setup can't bridge two strangers who only share the chain phrase. `distributed-topic-tracker` publishes each node's id into a Mainline DHT record keyed by a SHA-512 hash of the topic name, then bootstraps the gossip swarm from the record's public keys — exactly "same phrase → auto-connect". The topic name is derived from the chain fingerprint (stable across devices) and the per-device iroh identity + ULID are persisted next to the chain identity (`sync-node.json`, 0600) so restarts keep the same node id. The folder transport stays as the offline/air-gap fallback when the gossip topic has no peers yet. |

- Sync CRDT engine: automerge vs yrs (phase 8).
- LAN mDNS transport (zero-config local sync without internet) — ongoing (D60 chose iroh+DHT for cross-internet).
